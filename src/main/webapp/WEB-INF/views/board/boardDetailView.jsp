...
<%-- 아래의 자바스크립트 함수에서 사용할 url 변수 만들기 함 --%>
...
<c:url var="bdel" value="bdelete.do">
	...
	<c:param name="boardRenamerFileName" value="${ board.boardRenameFileName }"/>
</c:url>
	...
	<tr><th colspan="2">
		...
		<c:if test="${ !empty sessionScope.loginUser }">
			...
			<c:if test="${ loginUser.userId ne board.boardWriter or loginUser.adminYN eq 'Y' }">
				<%-- 글 레벨이 3보다 작은(2까지) 경우에만 댓글 달기 버튼 표시 : 대댓글까지만 등록하게 할 것임 --%>
				<c:if test="${ board.boardLev lt 3 }">
					<button onclick="requestReply(); return false;">댓글 달기</button> &nbsp;
				</c:if>
			</c:if>
		</c:if>
