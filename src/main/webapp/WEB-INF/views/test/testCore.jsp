...
<%-- <%@ taglib prefix="fmt" uri="https://jakarta.ee/jsp/jstl/core" %> --%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
...
<%
	int n1 = 111, n2 = 222;
%>
<c:set var="total" value="<%= n1 + n2 %>"/> <br>
total 변수 값 : $ { total } <br>
<hr>
...
<%-- ${param.전송온이름} == request.getParameter("전송온이름") 
	 실행 테스트 시에는 브라우저 주소 표시줄에 기존 url 뒤에 직접 입력함 : 
	 http://localhost:8080/first/moveCore.do?name=김철수
--%>
<hr>

<h4>c:if 태그 : if 문과 같은 처리를 하는 태그</h4>
...
<h4>c:forEach 태그 : for 문, for each 문 처리하는 태그</h4>
...
<a href="${tf }">testFmt.jsp 페이지로 이동</a>
<hr>
