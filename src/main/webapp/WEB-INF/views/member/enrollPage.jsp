...
function dupIdCheck(){
	// 입력한 사용자 아이디가 사용 가능한지 확인하는 함수
	// 서버측에 요청하고 응답받을 때 페이지 새로 고침되면 안됨 - 해결 : ajax 기술
	
	// jQuery.ajax({ settings });
	$.ajax({
		url: 'idchk.do',
		type: 'post',
		data: { userId: $('#userId').val() },	<%-- 전송 보낼 값 지정하는 속성임, { 전송이름: 전송값 } --%>
...
// 제이쿼리 : $(function(){ ... }); 축약형, jQuery(document).ready(function(){ ... }); 과 밑의 코드가 같음
window.onload = function(){
	// input type="file" 이 선택한 사진파일 이미지 미리보기 처리
	var photofile = document.getElementById('photofile');	// input file 태그 정보 가져옴
	photofile.addEventListener('change', function(event){
		...
		// 현재 선택한 사진파일은 클라이언트 컴퓨터에 있는 파일임, src 속성에 적용 불가
		
		// 클라이언트 컴퓨터에 있는 사진파일을 읽어들여서 출력되게 처리해야 함
		// 파일입력 -- 출력처리
		const reader = new FileReader(); 	// 자바스크립트의 파일 읽기 객체 생성
		// 람다(lambda) 스트림 방식 사용
		// 이벤트 콜백함수 실행구문(기존 방식) : reader.onload = function(e){ 읽어들이기 처리 };
		reader.onload = (e) => {	// e : event 객체
			photo.setAttribute('src', e.target.result);		// e.target : 이벤트 발생 대상
			photo.setAttribute('date-file', file-name);
		};
		reader.readAsDataURL(file);		// 읽어서 img 에 적용 출력		
...
<table id="outer" align="center" width="700" cellspacing="5" cellpadding="0">
	...
	<tr><th width="120">*아이디</th>
			...
			<td><input type="text" name="userId" id="userId" required> &nbsp;
						<%--form 태그 안에서는 input 타입의 button 을 사용해야 함 --%>
	...
	<tr><th>* 전화번호</th>
			<td><input type="tell" name="phone" required></td></tr>
	<tr><th>* 이메일</th>
