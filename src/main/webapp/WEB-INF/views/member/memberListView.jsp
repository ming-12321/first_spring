...
$(function(){
	...
	$('input[name=item]').on('change', function(){
		...
		$('input[name=item]').each(function(index){
			...
			if($(this).is(':checked')){
				// 체크 표시된 아이템에 대한 폼이 보이게 함
				$('form.sform').eq(index).css('display', 'block');
			}else{
				// 체크 표시 안 된 아이템에 대한 폼이 안 보이게 함
...
<fieldset id="ss">
   ...
   <input type="radio" name="item" id="uenroll">가입 날짜 &nbsp;
   <input type="radio" name="item" id="ulogok">로그인제한 여부 &nbsp;
</fieldset>
...
<%-- 성별 검색 폼 --%>
<form action="msearch.do" id="ugenorm" class="sform" method="get">
...
<form action="msearch.do" id="ulogokform" class="sform" method="get">
	...
	<fieldset>
	<legend>검색할 로그인제한 여부를 선택하세요.</legend>
