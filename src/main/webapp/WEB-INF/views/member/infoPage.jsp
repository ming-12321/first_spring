...
// 제이쿼리 : $(function(){ ... }); jQuery(document).ready(function(){ ... }); 과 같음
window.onload = function(){
   // input type="file" 이 선택한 사진 파일 이미지 미리보기 처리
   ...
   photofile.addEventListener('change', function(event){
      ...
      // 선택한 파일을 img 태그의 src 속성값으로 적용함 : 이미지 변경될 것임
      const photo = document.getElementById('photo');
      // photo.src = '서버측에 있는 이미지 파일의 상대 | 절대 경로/' + file.name; (서버측에 있는 이미지 파일임)
      // 현재 선택한 사진 파일은 클라이언트 컴퓨터에 있는 파일임, src 속성에 적용할 수 없음
      
      // 클라이언트 컴퓨터에 있는 사진 파일을 읽어들여서 출력되게 처리해야 함 
      // 파일 입력 -- 출력처리
      const reader = new FileReader();      // 자바스크립트의 파일읽기 객체를 생성함
      // 람다(lambda) 스트림 방식 사용
      // 이벤트 콜백함수 실행구문(기존 방식) : reader.onload = function(e){ 읽어들이기 처리 };
      reader.onload = (e) => {      // e : event 객체
         photo.setAttribute('src', e.target.result);      // e.target : 이벤트 발생 대상 (읽어들일 파일 정보)
         photo.setAtrribute('data-file', file.name);
      };
...
<table id="outer" align="center" width="700" cellspacing="5" cellpadding="0">
   ...
   <tr><th>* 이름</th>
         <td><input type="text" name="userName" id="userName" value="${ requestScope.member.userName }"></td></tr>
         <tr><th>* 성별</th>
            <td>
   ...
   <tr><th>* 전화번호</th>
         <td><input type="tell" name="phone" value="${member.phone  }"></td></tr>
   <tr><th>* 이메일</th>
