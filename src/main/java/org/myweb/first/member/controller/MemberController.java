...
import org.myweb.first.board.controller.BoardController;
...
//   } //login.do
   ...
   // input 태그에 name 속성의 이름을 dto 의 필드명(property)과 일치시키면,
...
//         // 뷰로 같이 전달할 정보를 뷰와 함꼐 내보내기 위해 사용함
...
//   }
   ...
   public String loginMethod(Member member, HttpSession session, SessionStatus status, Model model) {
      ...
      // 조회해 온 회원정보가 있고, 암호화된 패스워드와 로그인시 전송 온 패스워드가 일치한지 비교함
   ...
   // 전송 온 값 꺼내기 : String 변수 = request.getParameter("전송용이름");
   // 위의 코드를 처리해주는 어노테이션을 메소드 매개변수 위치에 사용할 수도 있음
...
//         // ajax 요청시 결과 응답하는 방식 : @ResponseBody 에 담아서 보내는 방식, 별도의 출력 스트림을 만들어서 내보내기 하는 방식
//         // 방식2, 별도의 출력스트림 연결해서 문자열 내보내기 방식 사용함 (반환자료형이 void 임)
//         
...
//   }
   ...
   // 업로드되는 파일은 따로 전송받음 => multipart 방식으로 전송 옴 => 스프링이 제공하는 MultipartFile 클래스를 사용해서
   // 받음
   // 비밀번호(패스워드) 암호화 처리 기능 추가
   @RequestMapping(value = "enroll.do", method = RequestMethod.POST)
    ...
    * 
    * @RequestParam("전송이름") 자료형 변수명 == request.getParameter("전송이름");
    */
   ...
   public String memberUpdateMethod(Member member, Model model, HttpServletRequest request,
      ...
      // 서비스 모델의 메소드 실행 요청하고 결과받기
      if (memberService.updateMember(member) > 0) { // 회원 정보 수정 성공시boardController
   ...
   // 관리자용 검색 기능 요청 처리용 메소드 (전송방식 GET, POST 둘 다 사용할 수 있도록 하면)
   ...
   public ModelAndView memberSearchMethod(HttpServletRequest request, ModelAndView mv) {
	   ...
	   // 총 페이지수 계산을 위해 검색 결과가 적용된 총 목록 개수 조회
	   int listCount = 0;
	   switch(action) {
	   ...
	   if(list != null && list.size() > 0) {
		   ...
		   mv.addObject("messsage", "회원 관리 검색 결과가 존재하지 않습니다.");
		   mv.setViewName("common/error");
	   }
